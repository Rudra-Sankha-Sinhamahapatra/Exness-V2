...
export function CandlestickChart() {
  ...
  const fetchCandleData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/api/v1/klines?asset=${selectedAsset}&interval=${selectedInterval}`
      );
  ...
  const calculatePriceChange = () => {
    ...
    const latestClose = parseFloat(latest.close as unknown as string);
    const previousClose = parseFloat(previous.close as unknown as string);
  ...
  const currentPrice = candleData.length > 0 ? parseFloat(candleData[candleData.length - 1].close as unknown as string) : 0;
  const selectedCurrency = CURRENCY_OPTIONS.find(c => c.value === selectedAsset);

  return (
    <div className="max-w-full mx-auto p-4 sm:p-6 bg-black rounded-lg shadow-lg">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 p-3 bg-zinc-950 rounded-md gap-2">
        ...
        <div className="text-xs sm:text-sm text-gray-400" suppressHydrationWarning>
            ...
            <table className="w-full text-xs sm:text-sm text-white">
              ...
              <tbody>
                {candleData.slice(-10).reverse().map((candle, index) => {
                  const open = parseFloat(candle.open as unknown as string);
                  const high = parseFloat(candle.high as unknown as string);
                  const low = parseFloat(candle.low as unknown as string);
                  const close = parseFloat(candle.close as unknown as string);
                  
                  return (
                    <tr key={index} className="border-b border-zinc-700">
                      <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm">
                        <div className="flex flex-col">
                          <span>{convertTimestamp(candle.start || "").toLocaleDateString()}</span>
                          <span className="text-zinc-500 text-xs" suppressHydrationWarning>
                            {convertTimestamp(candle.start || "").toLocaleTimeString()}
                          </span>
                        </div>
                      </td>
                      <td className="px-2 sm:px-4 py-2 text-right">${open.toFixed(2)}</td>
                      <td className="px-2 sm:px-4 py-2 text-right text-green-600">${high.toFixed(2)}</td>
                      <td className="px-2 sm:px-4 py-2 text-right text-red-600">${low.toFixed(2)}</td>
                      <td className={`px-2 sm:px-4 py-2 text-right font-medium ${close >= open ? 'text-green-600' : 'text-red-600'}`}>
                        ${close.toFixed(2)}
                      </td>
                    </tr>
                  );
